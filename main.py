...
async def wishlist(message: types.Message):
    ...
    text = (
        ...
        "Обязательно укажи, адрес какого именно пункта ты указал (Wildberries, Ozon или почта России), "
        "чтобы Вайшнавский Санта не запутался и заказал подарок там, где нужно 😉🎁\n"
